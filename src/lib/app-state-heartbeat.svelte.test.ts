...
describe('#191 initApp 統合挙動（ハートビート/サスペンド検出）', () => {
  ...
  it('awake-hidden → 復帰: 背景 interval が gap を詰め続けたので long-background モーダルは出ない', async () => {
    ...
    // 何度か繰り返す（＝マシンが起きていて背景タイマーが走り続けている状態）。
    for (let i = 0; i < 8; i++) {
      vi.advanceTimersByTime(30_000) // interval 発火 → lastHeartbeatTime = 現在(fake)時刻
    }
    // ここで fake now と lastHeartbeatTime はほぼ一致（gap ≈ 0）。visible 復帰イベントを流す。
