...
export interface RunResumeStaleCheckRetryOptions {
  ...
  runStaleCheck: (attemptIndex: number) => Promise<ApplyStaleResultOutcome>
  /** 現在タブが visible かどうか（呼び出し時点で評価する getter）。 */
  isVisible: () => boolean
 ...
 *   最終的な `check-failed` を返す。途中で `isVisible() === false` になって
...
export async function runResumeStaleCheckRetry(
  ...
  const { backoffsMs, runStaleCheck, isVisible, sleep = defaultSleep } = options
  let lastOutcome: ApplyStaleResultOutcome = 'check-failed'

  for (let i = 0; i < backoffsMs.length; i++) {
    await sleep(backoffsMs[i])
    if (!isVisible()) {
      // 再び hidden に戻ったら無駄な API 呼び出しを避ける。
      // 次回の visibility 復帰で初回 stale check からやり直される。
      return lastOutcome
    }
    const outcome = await runStaleCheck(i)
    lastOutcome = outcome
