...
| `color`                   | **非対応**                             | レポートに記録                                                                                                                                    |
| `attachments`             | メディアリポへ自動アップロード（#249） | zip 内の実体を解決 → `uploadMedia` → 本文末尾に `![name](rawURL)` を追記。実体が見つからない/メディア未設定/形式外（.3gp 録音等）はレポートに記録 |
| `labels`                  | **非対応**                             | レポートに記録                                                                                                                                    |
| `isPinned`                | **非対応**                             | レポートに記録                                                                                                                                    |
| `textContentHtml`         | **使用しない**                         | プレーンテキスト優先                                                                                                                              |
| `.html` ファイル          | **使用しない**                         | JSONのみ使用                                                                                                                                      |
| 画像ファイル              | 添付解決に使用（#249）                 | `attachments[].filePath` をパス完全一致 → ベース名一致で解決                                                                                      |
