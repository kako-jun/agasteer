...
- **ヘルプリンク**: 設定画面に使い方（テキスト・動画）へのリンクを追加（動画リンクは未公開URLだったため後に削除）
...
- **PR #48**: App.svelte 316行（ロジックを3モジュールに抽出）
...
## Phase 6: App.svelte からのロジック抽出（PR #48）

### 概要

App.svelte に集中していたナビゲーション・キーボード・PaneActions 生成ロジックを3つの専用モジュールに抽出し、App.svelte を316行まで削減しました。

### 新規モジュール

| ファイル                         | 行数 | 責務                                                                |
| -------------------------------- | ---- | ------------------------------------------------------------------- |
| `pane-navigation.svelte.ts`      | 747  | ペインナビゲーション（ビュー遷移・パンくず・URL同期・ワールド切替） |
| `keyboard-nav.svelte.ts`         | 161  | グローバルキーボードナビゲーション                                  |
| `pane-actions-factory.svelte.ts` | 603  | PaneActions 生成ファクトリ（Context API用）                         |

### 成果

- **App.svelte**: 316行（ロジック抽出前から大幅削減）
- ナビゲーション・キーボード・アクション生成の責務を明確に分離
- 各モジュールが単一責任を持ち、テスト・保守が容易に
