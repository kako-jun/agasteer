...
1. **~~エクスポート/インポート機能~~** ✅ **完了**
   ...
   - ✅ Google Keep形式からのインポート
   - ✅ Cosense (Scrapbox) 形式からのインポート

2. **~~検索機能~~** ✅ **完了（基本機能）**
   - ✅ ノート名・リーフ名での検索
   - ✅ 全文検索（content内）
   - ✅ アーカイブ内検索対応（アーカイブロード済みの場合のみ）
   - [ ] 正規表現対応（未実装）
...
1. **GitHub双方向同期**
   - ✅ GitHub→アプリの自動取得（自動Pull）
   - [ ] 競合解決UI
   - [ ] 差分表示

2. **Markdown拡張**
   - ✅ タスクリスト（Priorityリーフで実現: `[n]` マーカーによる優先度管理）
   - ✅ OCR機能（Tesseract.jsによるブラウザ内テキスト読み取り、カメラ/ファイル選択対応）
   - [ ] 数式サポート（KaTeX）
   - [ ] Mermaid図表対応
...
5. **ワールド機能の拡張** ✅ **一部完了**
   - ✅ ペインごとに独立したワールド表示（左:ホーム、右:アーカイブ）
   - ✅ アーカイブへの静的URL対応（パス埋め込み形式: `?left=archive:ノート>リーフ`）
   - ✅ 検索のアーカイブ対応（アーカイブロード済みの場合、検索対象に含める）
   - 詳細: [URLルーティングとワールド](./ui/routing.md)
...
4. **Gboard（Android）での空行タップ時のスクロールジャンプ**
   ...
   - タップ直後のCodeMirror自動スクロール抑制と、仮想キーボード表示後のカーソル可視範囲補正で緩和済み
   - OS/IME側の挙動を完全に制御することは困難（キーボードを非表示にすると問題は発生しないことを確認済み）
   - **回避策**: 空行をタップする代わりに、テキストがある行をタップしてからカーソルを移動する
   - 関連: Androidでは`highlightActiveLine()`が段落をまたぐ範囲選択で問題を起こすため除外済み（[features.md参照](./features.md#モバイル互換性)）
