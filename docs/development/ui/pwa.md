...
`handleVisibilityChange` はレイアウト修復に加えて、実サスペンドからの復帰も検出する。ハートビート interval（約 30 秒間隔）が刻む `lastHeartbeatTime` からの gap が閾値（5 分）を超えたとき（＝プロセスが凍結していたとき）だけ「長時間バックグラウンドにいました」モーダル + stale チェックを走らせる。単なるタブ切替・最小化では interval が gap を詰め続けるため発火しない。詳細は [`../sync/stale-detection.md`](../sync/stale-detection.md) の「長時間バックグラウンド復帰時の動作」節を参照。
