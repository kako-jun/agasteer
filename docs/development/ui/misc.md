...
- **通常トースト**: 表示から2秒で自動消滅。`showPushToast`（push スロット）は `pushToastState` を書き換えるたびに世代カウンタをインクリメントし、自動消滅タイマーは発火時に自分の世代が最新のときだけ状態を触るので、後から別の表示（同一文言の再表示・sticky・clear を含む）に差し替わっていれば何もしない（**後勝ち**、#308）。`showPullToast`（pull スロット）は表示のたびに前の自動消滅タイマーを解除（`clearTimeout`）してから新しい2秒タイマーを張る。消滅時刻は常に最後に出したトーストが決める（**後勝ち**、#302）。
...
- **トーストの幅**（#256 → #270）: `.toast`（`layout/Toast.svelte`）の幅は `width: max-content`・`max-width: calc(100vw - 24px)`・`overflow-wrap: anywhere` の3つで決まり、本文が画面幅に収まる間は1行に展開（カウントダウン時は本文1行＋数字1行＝計2行）、超えたら max-width でキャップして折り返す。この3プロパティのいずれかを外すと折返し／見切れのデグレになるため必ず3点セットで維持する（#256 で `width` 相当を失って3行化したのが #270 の実例）。
