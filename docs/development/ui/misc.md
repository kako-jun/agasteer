...
- **通常トースト**: 表示から2秒で自動消滅。push スロットの表示を差し替えるたび（`showPushToast`・sticky・clear）に世代カウンタをインクリメントし、自動消滅タイマーは発火時に自分の世代が最新のときだけ状態を触るので、後から別の表示（同一文言の再表示・sticky・clear を含む）に差し替わっていれば何もしない（**後勝ち**、#308）。`showPullToast`（pull スロット）は表示のたびに前の自動消滅タイマーを解除（`clearTimeout`）してから新しい2秒タイマーを張る。消滅時刻は常に最後に出したトーストが決める（**後勝ち**、#302）。
