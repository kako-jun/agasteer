...
- **同期層**（結果オブジェクト契約・throw しない）:
  ...
  - 副作用層 `api/media-library.ts`（media.ts が 400 行超のため sibling に分離）: `listMediaAssets`（`GET /repos/{repo}-media/git/trees/HEAD?recursive=1`。**404＝リポ未作成・409＝空リポは成功扱いで空配列**）、`deleteMediaAsset`（Contents API `DELETE`、sha 必須。成功後 mediaCache/mediaPending を evict）。いずれも 30 秒タイムアウト（#262）。
