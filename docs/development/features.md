...
2. `uploadMedia`（#242 同期層）: 検証（形式ホワイトリスト・100MB）→ URL 即時確定 → enqueue **で即返る**（#247）。実アップロードは待たず、背景タスク `uploadDone: Promise<boolean>` として返す。背景アップロードはグローバル直列チェーンで流す（Contents API は default branch へ直接コミットするため、同一リポへの並行 PUT は 409 になりうる。即返し＝並行化の衝突を回避）。チェーン経路の全 fetch にはタイムアウトがあり（#252: メタデータ系 30 秒・PUT は 60 秒 + 50KiB/s 換算のサイズ比例）、IndexedDB 操作（enqueue・dequeue・キャッシュ書き込み・リトライの getAll）も 10 秒で有限化している（#261: IDB のハングは request が settle しないまま止まるため try/catch では防げない。詳細は [storage.md](./storage.md) の mediaPending 節）ため、1 件のストールが以後の背景アップロードを止め続けること（head-of-line blocking）はない。タイムアウトしたアイテムは pending に残り、online 復帰・次回起動の `initMediaOnlineRetry` が回収する
...
- **履歴を残さないコミット方式**（#250・`media/history.ts`）: メディアは世代管理が不要なのに Contents API の変更はコミットを積み上げ、削除してもファイルが履歴に残ってリポが単調肥大する。そこでアップロード・削除の変更成立後に、その変更コミットと同じ tree を指す**親なしコミット**を作って ref を force 更新し（`collapseMediaHistory`、Git Database API 3 呼び出し）、リポを常に 1 コミット＝現在のファイルだけに保つ。手動メンテの UI は設けない（ユーザー操作なしで常に最適）。
  - **バッチ最適化**: 複数添付では自分の後ろに pending が残っている間は畳みを後続に委ね、最後の 1 回だけ実行する（中間の畳みは最後の畳みに完全に包含されるため）
  - **並行変更ガード**: ref 更新前に HEAD が自分の変更コミットのままか確認し、他デバイスの並行変更を検知したらスキップ（畳み損ねは次回の変更時に自動回収＝自己修復）。スキップ時は**自ファイルの存在を再確認**し、相手の force 差し替えに巻き込まれて HEAD から消えていた場合は dequeue せず pending に残す（リトライが dedup 経由で再アップロードして自己修復）
  - **本文読みの有限化**: チェーン経路の応答 JSON は `readJsonWithTimeout`（`media/http.ts`）で上限つきに読む。素の `res.json()` はヘッダ後の本文ストールで無限待ちになり #252 の head-of-line blocking 保証を破るため
  - 全段 best-effort で、失敗しても変更自体の成功は覆らない。必要権限は Contents の範囲内（Fine-grained トークンでも追加権限不要）。GitHub 側の容量表示への反映（unreachable オブジェクトの GC）は遅延することがある
