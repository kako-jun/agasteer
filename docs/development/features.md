...
- **履歴を残さないコミット方式**（#250）: メディアは世代管理が不要なのに Contents API の変更はコミットを積み上げ、削除してもファイルが履歴に残ってリポが単調肥大する。そこで**アップロード・削除のたびに**、その変更コミットと同じ tree を指す**親なしコミット**を作って ref を force 更新し（`collapseMediaHistory`、Git Database API 3 呼び出し）、リポを常に 1 コミット＝現在のファイルだけに保つ。手動メンテの UI は設けない（ユーザー操作なしで常に最適）。ガード: ref 更新前に HEAD が自分の変更コミットのままか確認し、他デバイスの並行変更を検知したらスキップ（畳み損ねは次回の変更時に自動回収＝自己修復）。全段 best-effort で、失敗しても変更自体の成功は覆らない。必要権限は Contents の範囲内（Fine-grained トークンでも追加権限不要）。GitHub 側の容量表示への反映（unreachable オブジェクトの GC）は遅延することがある。
