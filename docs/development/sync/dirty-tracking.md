...
2. Pullボタンに赤丸を表示
3. 確認ダイアログを表示（手動Pushと同じモーダル）
4. OKなら強制Push続行、キャンセルならタイマーリセットして終了
...
3. 5分経過 + ダーティあり + アクティブ
4. staleチェック
   - staleでない → Push実行 → `lastPushTime`更新 → `clearAllChanges()`
   - stale → `isStale = true` → Pullボタンに赤丸 → 確認ダイアログ → Pull first / Push上書き / キャンセル
   - キャンセル → `isStale = true`を維持し、Pullボタンの赤丸を残す
