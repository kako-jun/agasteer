...
Pull 操作（ボタンクリック、設定画面閉じる、定期 stale-pull、初回起動時の不一致/失敗時フォールバック）はこの関数を通ります。

**#158 以降の例外**: 起動時（`initApp` 内）にリモート HEAD SHA が `lastKnownCommitSha` と一致すると判定された場合は `pullFromGitHub` を呼ばずに IndexedDB から直接復元します（アプリのバージョンバンドルだけ変わった場合に全リーフ再取得を避けるため）。詳細は [`data-model.md` の「状態の初期化フロー」](../data-model.md) を参照。
