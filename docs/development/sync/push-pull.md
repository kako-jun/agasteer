...
8. #297 should5: `RS中`（rehydrateForRepo実行中）に ArchiveLoad/WorldSwitch を開始しても拒否（ブロック）はしない。ワールド表示自体は即座に切り替わるが、`handleWorldChange()`/`moveNoteToWorld()`/`moveLeafToWorld()`のアーカイブロード（IndexedDB読み書き）は開始前に`waitForRehydrate()`でrehydrateの完了を待ってから実行される（旧/新DBの取り違え防止）。
   ...
   - `handleWorldChange()`はワールド表示の即時切替（上記）を保つため、判定→`waitForRehydrate()`→**再判定**の順にする。冒頭の判定は待機前の状態（＝ワールド表示は即座に切り替える）、アーカイブロード開始直前にもう一度`isPulling || isPushing || isPushingBackground || isArchiveLoading || isArchiveLoaded`を判定し、該当すれば打ち切る。再判定なしで進むと、待機中に取られた Pull とアーカイブロードが並走（排他表崩壊）したり、両ペイン連続 Archive 操作で ArchiveLoad が二重起動したりする。
...
| 12b | `rehydrateForRepo()`実行中にアーカイブロード開始（`handleWorldChange`のワールド切替、`moveNoteToWorld`/`moveLeafToWorld`のWorld間移動）                         | rehydrate完了を待ってからアーカイブのIndexedDB読み書きが始まる。待機中に他の同期（Pull/Push/AL）がロックを取った、または別ペインのアーカイブロードが先に完了した場合はアーカイブロードを開始しない                                                                                | `moveNoteToWorld()`/`moveLeafToWorld()`は`waitForRehydrate()`を関数冒頭・Pull/Push/AL判定より前に置き、待機後の状態で判定を1回だけ行う。`handleWorldChange()`はワールド表示の即時切替（注8）を保つため、待機前に1回（表示切替の可否）、`waitForRehydrate()`をawait後にもう1回（`isPulling`/`isPushing`/`isPushingBackground`/`isArchiveLoading`/`isArchiveLoaded`の再判定）の計2回判定する（#297 must1）。判定→待機→再判定なしで進むと、`setCurrentRepo()`によるDB切替の途中でアーカイブの読み込み/保存が走り旧/新DBを取り違える、または待機中に取られた Pull とアーカイブロードが並走する（排他表崩壊）、連続 Archive 操作で AL が二重起動する |   A    |
