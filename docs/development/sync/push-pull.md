...
| **アーカイブPull（restoreStateFromUrl）** | 許可 | 許可（注8）   | 許可（注8） | 許可（注8）   | 許可                   |
...
8. `handleWorldChange()`と異なり「ブロックして即 return」ではない（#314）。`restoreStateFromUrl()`は deep-link で開いた URL を必ず解決する必要があるため、Pull中/Push中/ArchiveLoad中は`waitForSyncIdle()`/`archiveLoadInFlight`（performArchiveLoadの再入デデュープ）で busy が解消するまで待ってから続行する
...
8. #297 should5: `RS中`（rehydrateForRepo実行中）に ArchiveLoad/WorldSwitch を開始しても拒否（ブロック）はしない。ワールド表示自体は即座に切り替わるが、`handleWorldChange()`/`moveNoteToWorld()`/`moveLeafToWorld()`のアーカイブロード（IndexedDB読み書き）は開始前に`waitForRehydrate()`でrehydrateの完了を待ってから実行される（旧/新DBの取り違え防止）。
   ...
   - #314: `performArchiveLoad()` 自体を再入安全にした。`handleWorldChange()`のロード進行中に popstate 等で`restoreStateFromUrl()`からも呼ばれると、従来は`pullArchive()`が2本走り、先に終わった方の`finally`が`isArchiveLoading=false`にしてもう片方の`pullArchive()`実行中にロックを外してしまっていた（二重ロード）。`archive-load.svelte.ts`が進行中ロードの Promise（`archiveLoadInFlight`）を保持し、再入時は新しいロードを始めずその Promise を返して完了を待つことで解消した。加えて`restoreStateFromUrl()`自身もアーカイブロード前に`waitForRehydrate()`と Pull/Push（背景含む）のアイドル待ち（`waitForSyncIdle()`、ポーリング。pane-navigation.svelte.ts）を経るようになり、`handleWorldChange()`と同じ前段（rehydrate待ち→Pull/Push非併走）に揃った。`handleWorldChange()`は busy ならワールド表示を home に戻して諦めるが、`restoreStateFromUrl()`は URL を必ず解決する必要があるためスキップせず待つ（onPriorityComplete からは`await`されずに呼ばれるため、この待機は呼び出し元の Pull 自体をブロックしない。デッドロックしないことをコードで確認済み）。
...
| `isArchiveLoading`                                                                  | `pullFromGitHub()`/`pushToGitHub()`/`moveNoteToWorld()`/`moveLeafToWorld()`が冒頭で個別チェックし、`handleWorldChange()`は冒頭のbusy判定とアーカイブロード直前の再判定の2箇所でチェックする。`restoreStateFromUrl()`は`!isArchiveLoaded && token && repoName`をロード開始前と rehydrate/Pull-Push 待機後の2回判定する（#314、`handleWorldChange()`の再判定と同じ考え方）。取得・解除は、`handleWorldChange()`/`restoreStateFromUrl()`経由なら`performArchiveLoad()`がIndexedDB読み出し前に同期的に行い、`moveNoteToWorld()`/`moveLeafToWorld()`はアーカイブ事前Pull時に自前で取得し各関数自身のfinally句で解除する（#297 S-b / #307）。`performArchiveLoad()`自体が再入安全（#314）: 実行中に再度呼ばれても新しいロードは始めず、進行中の Promise（`archiveLoadInFlight`）を返す。このほか自動Push・自動Pull・stale checkリトライ（visibility復帰/online復帰）・`handleSettingsChange()`・`shouldQueueRepoSync()`（`handleCloseSettings()`等）も参照する（詳細は「アーカイブロード中の操作制限」節等） | Pull, Push, WS, 移動                    |
...
| Pull/Push/背景Push 実行中（`restoreStateFromUrl()`のみ、`waitForSyncIdle()`、#314） | `restoreStateFromUrl()`のアーカイブロード直前（`waitForRehydrate()`の後）。`isPulling`/`isPushing`/`isPushingBackground`は Svelte ストアの`subscribe`を持たない`$state`のため、`pane-navigation.svelte.ts`内でポーリングして待つ（`sync/resume-retry.ts`の sleep 注入と同じ考え方）。他の行と異なり**ブロックして即 return するのではなく、アイドルになるまで待ってから続行する**（URL を必ず解決する必要があるため）。onPriorityComplete（git-pull.ts）から`await`されずに呼ばれる経路があるため、この待機は Pull 自体をブロックしない                                                                                                                                                                                                                                                                                                                                                                                                                                                                | URL復元（アーカイブ要求時のみ）         |
