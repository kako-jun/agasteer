...
| Pull/Push を理由にした再判定拒否（#297 4巡目） | `handleWorldChange()`内（`pane-navigation.svelte.ts`）。ワールド表示を切替前（home）へ直接戻す（自動再開はしない）                                                                                                                                                                                                | ワールド表示の残留防止         |
