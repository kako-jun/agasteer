...
3. **メディア添付の挿入フェーズ着地を待つ**（`waitForPendingMediaInserts()`、上限つき。#254）
4. **全エディタの IME composition を強制 flush** + `tick()` で reactive 更新を 1 サイクル待つ（#186）
5. 保留中の自動保存をフラッシュ
6. Staleチェック（共通関数で時刻も更新）。`check_failed` のとき、`abortIfStaleCheckFailed` オプション指定時（auto-push）は中止（#235）
7. Stale編集の場合はまず push-in-flight 救済（`tryRescueStalePush()`）を試し、成立すればダイアログなしで続行。不成立なら確認（ロックを保持したまま await）（#235）
8. Push飛行中フラグを設定（`pushInFlightAt`。設定した stamp を控えておく）
9. **Push実行を `Promise.race` で 30 秒タイムアウト付きで待つ**（`PUSH_TIMEOUT_MS`、#204）
10. Push飛行中フラグをクリア（**タイムアウト時は意図的に残し**、`observeOrphanPush()` で遅延結果を観測する。#235）
11. 結果処理（成功時はダーティクリア、リモートからpushCount取得）
12. ロック解放（finallyで必ず実行）

**ステップ 9（30 秒タイムアウト）の意義（#204）:**

スマホで Push 中にスリープすると、`fetch` の Promise が pending のまま戻らず、`finally` まで到達できないため `isPushing` が永遠に `true`、ガラス効果と `inert` が UI に残り続ける。`Promise.race` で 30 秒経ったら `PushTimeoutError` を投げて `isPushing` を解除し、warning トースト（`toast.pushTimeout`、E-5101）を出す。`pushInFlightAt` は残し、race に負けた `executePush` の Promise には `observeOrphanPush()` で continuation を接続する（#235）。遅延成功なら settle した瞬間に `lastKnownCommitSha` を追従させてトースト（`toast.pushLateSuccess`）で通知し、遅延失敗 / reject は warn ログに落とす（unhandledrejection 解消）。`pushInFlightAt` の遅延クリアは「自分が設定した stamp と一致する場合のみ」の guarded clear で、後続 Push が設定した救済マーカーは壊さない。ページリロードで観測が消えた場合は、従来どおり次回 stale check の push-in-flight 救済（`tryRescueStalePush`）が保険になる（詳細は `stale-detection.md`）。

`AbortController` を fetch 層まで通すコストは大きいため採らず、Promise.race の loser タイマーは `clearTimeout` で確実に片付ける。

**`PushToGitHubOptions`（#235）:**

`pushToGitHub(options?: PushToGitHubOptions)` は `abortIfStaleCheckFailed` オプションを受け取る。`true` のとき、ステップ 6 の stale check が `check_failed`（ネットワーク断・認証エラー等で判定不能）なら Push を中止する。手動 Push（既定 `false`）は「ユーザーが押した」意思を尊重してそのまま続行するが、auto-push は無人発火のため中止し、オフライン中に 42 秒ごとのエラートースト連発になるのを防ぐ（auto-push の自前 stale check を preflight に一本化した後も、旧来の「check_failed は静かにスキップ」挙動を維持するためのオプション）。

**ステップ 3（メディア挿入フェーズ待ち）の意義（#254）:**

メディア添付（`attachMediaFiles`）は「画像最適化 → ハッシュ確定 → enqueue → 記法挿入」の順で進み、挿入はローカル処理の完了後に走る。モバイルでは最適化に数百 ms〜数秒かかるため、添付直後に Push すると snapshot が挿入前の内容を固定して「変更なし」で no-op になり、アップロード済みメディアへの参照テキストだけがリポに保存されない窓ができていた（次回 Pull で参照が消えるとメディアが孤児化）。

`src/lib/api/media/insert-phase.ts` が挿入フェーズ（ネットワーク待ちを含まないローカル区間のみ）を in-flight 追跡し、Push / Pull / リポ切替（`rehydrateForRepo`）の preflight が `waitForPendingMediaInserts()` で着地を待つ。待ちには上限（`MEDIA_INSERT_WAIT_TIMEOUT_MS` = 10 秒、`sync/constants.ts`）があり、超過時は warn を出して従来挙動（挿入前の内容で続行）に落ちる — 同期を永久に塞がないための保険。さらに、開始から `MEDIA_INSERT_PHASE_STALE_MS`（60 秒）を超えてなお未完了のフェーズはタイムアウト時に追跡から隔離し、settle しないフェーズ 1 つで以後の全同期が毎回 10 秒待たされ続ける事態を防ぐ（正当に遅いだけのバッチは閾値内なら保護が継続する）。

既知の限界: 合計処理が 10 秒を超える巨大バッチの最中に Push した場合は従来挙動に落ちるため、#254 の窓が残る（リトライ Push で回収可能）。挿入待ちは IME flush より**前**に置く — 挿入が composition 中に着地したケースを後段の flush が store へ反映する順序のため。

**ステップ 4（IME flush）の意義（#186）:**
...
flowchart TD
    ...
    Lock --> MediaWait[メディア挿入フェーズ待ち<br/>上限つき #254]
    MediaWait --> FlushIME[全エディタのIME composition flush<br/>+ tick]
    FlushIME --> Flush[保留中の保存をフラッシュ]
    Flush --> Stale[Staleチェック]
    Stale --> CheckFail{check_failedかつ<br/>abortIfStaleCheckFailed?}
    CheckFail -->|Yes: auto-push経由 #235| Unlock
    CheckFail -->|No| Check2{Stale?}
    Check2 -->|Yes| Rescue{push-in-flight<br/>救済成立? #235}
    Check2 -->|No| Push
    Rescue -->|Yes: 誤検出→続行| Push
    Rescue -->|No| Confirm[3択ダイアログ<br/>ロック保持]
...
1. 交通整理: Pull/Push中またはアーカイブロード中は不可
2. 即座にロック取得
3. **メディア添付の挿入フェーズ着地を待ち、IME composition を flush**（Push 側と同じ #254 / #186 対策。挿入が store に反映される前に下のダーティチェックが clean を誤認して上書き Pull すると、挿入テキスト＝メディア参照が消える）
4. Staleチェック（リモートが `up_to_date` かつ Pull完了済みなら早期リターン。実Pullが走らないパスで誤った「上書き警告」を出さないため、#152）
   - **#158**: 呼び出し元で既に stale check 済みなら `precomputedStale` 引数で結果を渡せる。GitHub Refs API の二重呼び出しを避ける
5. ダーティチェック（ロックを保持したまま確認ダイアログ。実Pullが走る可能性がある場合のみ）
6. Pull実行（第一優先で編集可能に、残りはバックグラウンド）
7. 結果処理（成功時はpushCount更新、ダーティクリア）
...
flowchart TD
    ...
    Lock --> MediaWait[メディア挿入フェーズ待ち<br/>+ IME flush #254]
    MediaWait --> Stale[Staleチェック]
    Stale --> Check3{変更あり?}
...
ダーティな状態で**最後のユーザー操作から42秒**（`AUTO_PUSH_INTERVAL_MS`）無操作が続くと`shouldAutoPush`が立ち、以下の条件をすべて満たす場合に`pushToGitHub()`を呼び出します：

1. タブがアクティブ（`document.visibilityState === 'visible'`）
2. GitHub設定済み
3. Pull/Push中でない（background push 含む）、かつアーカイブロード中でない
4. 初回Pull完了済み（`isFirstPriorityFetched`）

自動Pushも`pushToGitHub()`を呼ぶため、手動Pushと完全に同じ排他制御が適用されます。

### stale判定の preflight 一本化（#235）

以前は auto-push 側でも自前の stale check + 衝突ダイアログを実行してから `pushToGitHub()` を呼んでいたが、「Pushでリモートを上書き」を選んでも preflight が再び同じ stale を検知して**同一ダイアログが2連続で表示**され、GitHub Refs API の二重呼び出しにもなっていた。現在は `pushToGitHub({ abortIfStaleCheckFailed: true })` の1呼び出しに一本化し、stale 判定・push-in-flight 救済・衝突ダイアログ（選択肢と診断情報を含む）はすべて preflight 側が担う。`abortIfStaleCheckFailed: true` は無人発火用のオプションで、`check_failed`（オフライン等）のときは Push を強行せず静かに中止する（旧実装の「静かにスキップ」挙動を維持）。

タイマー（`resetAutoPushTimer()`）は Push 試行の**前に**リセットする。preflight のダイアログでキャンセルされたり Push が失敗して dirty のまま残っても、1秒ごとの再発火にならず次の試行は42秒後になる（旧実装の cancel / check_failed 時と同じ間隔）。

### 自動Pushフロー

```mermaid
flowchart TD
    Timer[無操作42秒で<br/>shouldAutoPush] --> Active{アクティブ?}
    Active -->|No| Skip[スキップ]
    Active -->|Yes| Config{GitHub設定?}
    Config -->|No| Skip
    Config -->|Yes| Sync{Pull/Push/<br/>ArchiveLoad中?}
    Sync -->|Yes| Skip
    Sync -->|No| First{初回Pull完了?}
    First -->|No| Skip
    First -->|Yes| Reset[タイマーリセット]
    Reset --> Push[pushToGitHub<br/>abortIfStaleCheckFailed: true]
    Push --> Note[stale判定・救済・衝突ダイアログは<br/>preflight側で実施 #235]
    Note --> End[完了]
```
...
| **アーカイブPull（restoreStateFromUrl）** | 許可 | 待機（注8）   | 待機（注8） | 待機（注8）   | 待機（注8）            |
...
8. `handleWorldChange()`と異なり「ブロックして即 return」ではない（#314）。`restoreStateFromUrl()`は deep-link で開いた URL を必ず解決する必要があるため、Pull中/Push中/ArchiveLoad中（リポ切替直後の rehydrate 実行中を含む）は busy が解消するまで待ってから続行する。待機は`waitForRehydrate()`（rehydrate専用、#297）→`waitForSyncIdle()`（Pull/Push/背景Push/ArchiveLoad、#314 S1: 対象フラグの setter からの通知で待つ。ポーリングではない）の2段（#314 M2: `waitForSyncIdle()`の busy 判定に`isArchiveLoading`も含むため、`performArchiveLoad()`を経由しない自前ロード（`moveNoteToWorld()`/`moveLeafToWorld()`）が進行中でも二重ロードしない）。archive を必要としない（または既にロード済みの）pane は待たずに即解決し、archive 待ちが要る pane だけ待機後に解決する（#314 M4a）
...
| **RS中**   | 自動実行 | ブロック | 許可（注8）     | 許可（注8）     | -              |

※ AL = ArchiveLoad、WS = WorldSwitch、RS = RepoSwitch

8. #297 should5: `RS中`（rehydrateForRepo実行中）に ArchiveLoad/WorldSwitch を開始しても拒否（ブロック）はしない。ワールド表示自体は即座に切り替わるが、`handleWorldChange()`/`moveNoteToWorld()`/`moveLeafToWorld()`のアーカイブロード（IndexedDB読み書き）は開始前に`waitForRehydrate()`でrehydrateの完了を待ってから実行される（旧/新DBの取り違え防止）。
   - #297 must1: `moveNoteToWorld()`/`moveLeafToWorld()`は`waitForRehydrate()`を関数冒頭・Pull/Push/ArchiveLoad判定より前に置く（判定は待機後の状態で1回だけ行う。pull/push preflightと同じ形）。
   - `handleWorldChange()`はワールド表示の即時切替（上記）を保つため、判定→`waitForRehydrate()`→**再判定**の順にする。冒頭の判定は待機前の状態（＝ワールド表示は即座に切り替える）、アーカイブロード開始直前にもう一度`getWorldForPane(pane) === 'archive'`・設定の有効性・`isPulling || isPushing || isPushingBackground || isArchiveLoading || isArchiveLoaded`を判定する（#297 N-a）。ペインが archive 表示でなくなった／設定が無効化された／AL中／既にロード済みなら、そのまま打ち切って何もしない。
   - #297 4巡目: 再判定が`isPulling || isPushing || isPushingBackground`を理由に打ち切った場合だけは、Pull/Push はアーカイブをロードしないため、そのまま終わるとワールド表示は切り替わっているのにアーカイブが未ロード（`isArchiveLoaded=false`、画面が空）のまま残ってしまう。以前は`appState.pendingArchiveLoad`を立てて同期完了後に自動再開していたが、保留中の rehydrate を無視して旧リポの IndexedDB に新リポのアーカイブを保存する／例外が Pull/Push に伝播する／配線が未テストといった新たなバグを生んだため撤去した。現在はこの場合、そのペインのワールド表示を切替前（= home）に戻す（`leftWorld`/`rightWorld`を直接更新し、view が `home` のままなら`goHome()`（待機中にメディア画面等へ移っていたら上書きしない、#297）、その後`refreshBreadcrumbs()`。`showPullToast(toast.archiveOpenBlocked)`で開けなかったことを案内する。`handleWorldChange()`を再帰呼び出しはしない。busy 判定で即 return し戻し処理自体が発火しないため）。これにより冒頭の busy 判定がワールド切替自体を拒否したときと同じ結果になり、「archive 表示なのに未ロード」の残留状態を作らない。ただし別ペインの AL が進行中/ロード済み（直前の判定分岐）の場合は戻さない（その場合は表示が正しく埋まるため）。待機中にユーザー操作でそのペインが既に別ワールドへ移っていた場合（1つ上の判定分岐）も触らない。
   - #297 S-b: アーカイブロードのロック（`appState.isArchiveLoading = true`）は、この再判定を通過した直後・IndexedDB読出し（`loadArchiveCacheFromDB()`）を`await`する前に同期的に取得する。読出し完了後に取得すると、読出し中はロックが無い窓ができ、その間に割り込んだ Pull がアーカイブロードと並走できてしまう。`restoreStateFromUrl` の URL 復元時アーカイブロードも `performArchiveLoad()` 経由で同じタイミングでロックを取る（#307）。
   - #314: `performArchiveLoad()` 自体を再入安全にした。`handleWorldChange()`のロード進行中に popstate 等で`restoreStateFromUrl()`からも呼ばれると、従来は`pullArchive()`が2本走り、先に終わった方の`finally`が`isArchiveLoading=false`にしてもう片方の`pullArchive()`実行中にロックを外してしまっていた（二重ロード、M3）。`archive-load.svelte.ts`が進行中ロードの Promise（`archiveLoadInFlight`）を保持し、再入時は新しいロードを始めずその Promise を返して完了を待つことで解消した。この`archiveLoadInFlight`の寿命はロックが立っている期間と厳密に一致させている（`isArchiveLoading=false`にするのと同じ`finally`内で`null`に戻す。予約同期の再開（`runPendingRepoSyncIfIdle()`）はその外側、最初の呼び出し元の流れで行う）。
   - #314 M2: `restoreStateFromUrl()`自身もアーカイブロード前に`waitForRehydrate()`と Pull/Push（背景含む）/ArchiveLoad のアイドル待ち（`waitForSyncIdle()`、S1: 対象フラグの setter からの通知で待つ。ポーリングではない）を経るようになり、`handleWorldChange()`と同じ前段（rehydrate待ち→他同期非併走）に揃った。`waitForSyncIdle()`の busy 判定に`isArchiveLoading`を含めることで、`performArchiveLoad()`を経由しない自前ロード（`moveNoteToWorld()`/`moveLeafToWorld()`）が進行中の場合も、その完了（`isArchiveLoaded=true`）を待ってから解決し、自分では`pullArchive()`を呼ばない（move.ts 側は変更不要）。`handleWorldChange()`は busy ならワールド表示を home に戻して諦めるが、`restoreStateFromUrl()`は URL を必ず解決する必要があるためスキップせず待つ。
   - #314 M1（デッドロック回避）: 起動時 Pull（`pullFromGitHub(true, onCancel, ...)`）のキャンセル分岐（`showConflictDialog`で pull 以外を選択）は、`onCancel?.()`を`await`する前に`isPulling.value = false`にする。`onCancel`（`applyPersistedStartupCache`経由）は`restoreStateFromUrl()`を`await`し、その中の`waitForSyncIdle()`は`isPulling`が`false`になるのを待つ。先に解放しないと、Pull 側の`finally`（`isPulling`解放）は`onCancel`の完了を待ち、`onCancel`は`isPulling`解放を待つ、という相互待機になる。一方、`onPriorityComplete`（Pull 途中の第1優先リーフ取得完了コールバック）からの呼び出しは型上`() => void`で`executePull()`から`await`されない fire-and-forget のため、この経路の`restoreStateFromUrl()`の待機は Pull 自身の完了（`isPulling`解放）をブロックしない。通常時（非初回）Pull の Push first / Cancel の3択でも同じ理由で、`pushToGitHub()`/`onCancel?.()`を`await`する前に`isPulling.value = false`にする（`showConflictDialog`で pull 以外を選択した全分岐が対象）。この2経路（await する onCancel / await しない onPriorityComplete）×起動時/通常時のどちらもデッドロックしないことをテストで固定している（`src/lib/actions/git-pull-restore-deadlock.test.ts`）。
   - #314 S1: 上記の早期解放（cancel/push-first 分岐）は`isPulling.value = false`にした後、別の Pull がそのロックを取ることがある。`pullFromGitHub()`末尾の`finally`が無条件に`isPulling.value = false`をもう一度実行すると、その間に他の Pull が取ったロックを誤って消してしまう。`let lockReleased = false`を用意し、早期解放時に`true`にして、`finally`は`if (!lockReleased) isPulling.value = false`にする（`git-pull-restore-deadlock.test.ts`で固定）。
   - #314 M4: `restoreStateFromUrl()`は呼び出しごとに世代カウンタを持ち、すべての`await`の後で世代が変わっていたら（＝より新しい呼び出しに上書きされていたら）何もせず抜ける。加えて呼び出し時点の pane 状態（world/note/leaf/view）のスナップショットと待機後の状態を比較し、ユーザーが待機中にそのペインを操作していれば上書きしない。archive を必要としないペインは待たずに即解決する（M4a、home pane が Pull 全体を待つ退行を防ぐ）。
   - #314 Q1: `onPriorityComplete`は`appState.isRestoringFromUrl`を以前は`isInitialStartup`のときだけ手動で true→false トグルしていたが、この呼び出しは未`await`のため、`restoreStateFromUrl()`がアーカイブ待機に入った続きが実行される前に false へ戻してしまい、待機後の pane 解決が`updateUrlFromState()`の抑制をすり抜けて`history.pushState`を1つ余分に積む窓があった。`restoreStateFromUrl()`自身が世代カウンタ（M4）でフラグの生死を待機を含めて最後まで管理するため、この手動トグルは撤去した（非初回パスと同じ形に統一）。
   - #314 S-A: `applyPersistedStartupCache()`側にも同種のトグルが残っていた。起動時（`initialStartup`）だけ`isRestoringFromUrl`を手動 true→false し、`restoreStateFromUrl(true)`に`alreadyRestoring`引数を渡していたが、これも Q1 と同じ窓（世代2が待機中に世代1側の finally が先に false へ戻す）を作りうるため、手動トグルと`alreadyRestoring`引数を撤去し`await deps.restoreStateFromUrl()`一本に統一した。呼び出しごとの生死管理を`restoreStateFromUrl()`自身の世代カウンタに一本化する、という設計を全呼び出し元で揃えた形。
   - #314 M-1: 上記の「世代カウンタでフラグの生死を管理する」は、本体を`try/finally`で包んで初めて成り立つ。旧形式 URL の早期`return`と、`waitUntilArchiveReady()`内で例外（IndexedDB reject 等）が飛ぶ経路の両方が、`try`の中に入っている必要がある。`finally`では「今も最新世代である呼び出し」だけが`appState.isRestoringFromUrl = false`にする。末尾に無条件で`false`を代入する古い実装だと、早期`return`や例外の経路がこの行に届かず`true`が残ったままになり、以後`updateUrlFromState()`（App.svelte の`$effect`から呼ばれる）が早期`return`し続けて URL・履歴が一切更新されなくなる（`pane-navigation-restore-archive-lock.test.ts`で固定）。呼び出し元（`onPriorityComplete`・popstate ハンドラ等）は`restoreStateFromUrl()`を`await`しないため、この経路で投げられた例外は明示的に`.catch()`しないと未処理の Promise rejection になる（`git-pull.ts`の2箇所・`app-state.svelte.ts`の popstate ハンドラで対応）。
...
| `isArchiveLoading`                                                                              | `pullFromGitHub()`/`pushToGitHub()`/`moveNoteToWorld()`/`moveLeafToWorld()`が冒頭で個別チェックし、`handleWorldChange()`は冒頭のbusy判定とアーカイブロード直前の再判定の2箇所でチェックする。`restoreStateFromUrl()`は待機ループ（`waitForRehydrate()`→`waitForSyncIdle()`、busy 判定に`isArchiveLoading`を含む）の同期再判定で`isArchiveLoaded`/設定の有効性を判定する（#314 M2/M4a、`handleWorldChange()`の再判定と同じ考え方。`moveNoteToWorld()`/`moveLeafToWorld()`のような`performArchiveLoad()`を経由しない自前ロードが進行中でも、この busy 判定で待ってから解決するため二重ロードしない）。取得・解除は、`handleWorldChange()`/`restoreStateFromUrl()`経由なら`performArchiveLoad()`がIndexedDB読み出し前に同期的に行い、`moveNoteToWorld()`/`moveLeafToWorld()`はアーカイブ事前Pull時に自前で取得し各関数自身のfinally句で解除する（#297 S-b / #307）。`performArchiveLoad()`自体が再入安全（#314 M3）: 実行中に再度呼ばれても新しいロードは始めず、進行中の Promise（`archiveLoadInFlight`。寿命はロック期間と一致）を返す。このほか自動Push・自動Pull・stale checkリトライ（visibility復帰/online復帰）・`handleSettingsChange()`・`shouldQueueRepoSync()`（`handleCloseSettings()`等）も参照する（詳細は「アーカイブロード中の操作制限」節等） | Pull, Push, WS, 移動                    |
| rehydrate実行中（`waitForRehydrate()`）                                                         | `pullFromGitHub()`/`pushToGitHub()`冒頭（`canSync`判定より前）、`moveNoteToWorld()`/`moveLeafToWorld()`冒頭（Pull/Push/AL判定より前）、`handleWorldChange()`のアーカイブロード直前（判定→待機→再判定）、`restoreStateFromUrl()`のアーカイブロード直前（#314。`handleWorldChange()`と同じ前段に揃えた）、`runPendingRepoSyncIfIdle()`/`handleCloseSettings()`冒頭（`shouldQueueRepoSync`判定より前、#297 should2）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Pull, Push, WS, 移動, URL復元, 予約同期 |
| Pull/Push/背景Push/ArchiveLoad 実行中（`restoreStateFromUrl()`のみ、`waitForSyncIdle()`、#314） | `restoreStateFromUrl()`のアーカイブロード直前（`waitForRehydrate()`の後）。`isPulling`/`isPushing`/`isPushingBackground`（core-state.svelte.ts）/`isArchiveLoading`（app-state.svelte.ts）は Svelte ストアの`subscribe`を持たない`$state`だが、setter が`stores/sync-signal.ts`の`notifySyncActivityChanged()`を呼ぶため、`waitForSyncIdle()`（`pane-navigation-url-restore.svelte.ts`）はポーリングせず通知で待つ（#314 S1）。他の行と異なり**ブロックして即 return するのではなく、アイドルになるまで待ってから続行する**（URL を必ず解決する必要があるため）。onPriorityComplete（git-pull.ts）から`await`されずに呼ばれる経路があるため、この待機は Pull 自体をブロックしない。起動時キャンセル（`onCancel`）のように`await`される経路は、キャンセル確定時に`isPulling`を先に解放してから呼ぶことでデッドロックを避ける（#314 M1。両経路とも`src/lib/actions/git-pull-restore-deadlock.test.ts`で固定）                                                                                                                                                                                                                                                                                                                                                 | URL復元（アーカイブ要求時のみ）         |
...
| `metadata`                 | `$state<Metadata>`         | `{version:1,notes:{},leaves:{},pushCount:0}`（モジュール初期値。`initApp()`が`setCurrentRepo()`直後・Pull前に`getPersistedMetadata()`で上書き。#295） | `metadataFromGitHub`で全体を上書き            | noChanges以外の成功時、`fetchRemotePushCount()`成功なら`pushCount`のみ更新（他フィールドは不変。#293） | `await getPersistedMetadata()`でIndexedDBの新リポキーから非同期再取得、なければ`{version:1,notes:{},leaves:{},pushCount:0}` | 旧リポのmetadata（pushCount含む）が新リポに残り、統計表示や衝突ダイアログのローカルpushCount比較が旧リポの値のまま誤表示される |
...
| 12b | `rehydrateForRepo()`実行中にアーカイブロード開始（`handleWorldChange`のワールド切替、`moveNoteToWorld`/`moveLeafToWorld`のWorld間移動）                         | rehydrate完了を待ってからアーカイブのIndexedDB読み書きが始まる。待機中に他の同期（Pull/Push/AL）がロックを取った、または別ペインのアーカイブロードが先に完了した場合はアーカイブロードを開始しない。**Pull/Push が理由で打ち切った場合は、そのペインのワールド表示を切替前（home）に戻し、案内トーストを出す**（#297 4巡目。自動再開はしない）。**AL が理由（進行中/ロード済み）で打ち切った場合は戻さない**（表示が正しく埋まるため） | `moveNoteToWorld()`/`moveLeafToWorld()`は`waitForRehydrate()`を関数冒頭・Pull/Push/AL判定より前に置き、待機後の状態で判定を1回だけ行う。`handleWorldChange()`はワールド表示の即時切替（注8）を保つため、待機前に1回（表示切替の可否）、`waitForRehydrate()`をawait後にもう1回（ペインがまだ archive 表示か・設定の有効性・`isPulling`/`isPushing`/`isPushingBackground`/`isArchiveLoading`/`isArchiveLoaded`の再判定、#297 N-a）の計2回判定する（#297 must1）。判定→待機→再判定なしで進むと、`setCurrentRepo()`によるDB切替の途中でアーカイブの読み込み/保存が走り旧/新DBを取り違える、または待機中に取られた Pull とアーカイブロードが並走する（排他表崩壊）、連続 Archive 操作で AL が二重起動する。Pull/Push を理由に打ち切った場合は`appState.pendingArchiveLoad`による自動再開を4巡目で撤去し、`leftWorld`/`rightWorld`を直接 home へ戻す方式に変更した（保留中の rehydrate を無視して旧DBに新リポのアーカイブを保存する／例外が Pull/Push に伝播する等の別バグを生んだため）。アーカイブロード完了時の保留同期消化（`runPendingRepoSyncIfIdle()`）は、以前は`handleWorldChange()`/`moveNoteToWorld()`/`moveLeafToWorld()`がそれぞれ`waitForRehydrate()`も`pendingRehydrateRepo`のrehydrateもしない簡略版を複製していたが、`git-pull.ts`の実装（正本。`pendingRehydrateRepo`があれば予約pull前にrehydrateする）に一本化した（#297 S-c。`git-push.ts`と同じ import 形） |   A    |
| 12c | `applyRehydrateForRepo()`内の未保護await（`getPersistedMetadata()`等）が例外を投げた直後に別リポへの切替要求が来ている                                          | 例外が起きてもキュー済みの最後の要求（最終的に必要なリポ）は破棄されず必ず適用される。reject するかどうかは**最終周回（キューが空になった時点の周回）が成功したか**だけで決まる（#297 question1）。途中の周回が失敗しても後続の周回が成功すれば`rehydrateForRepo()`はrejectしない                                                                                                                                                      | `rehydrateForRepo()`が各周回の例外を捕捉し、周回ごとに`lastError`をリセットして継続する（#297 must1/question1）。中間周回の失敗は内部でログするに留め、最終周回の失敗だけをまとめてrejectする（呼び出し元側の`.catch`/`try-catch`と二重ログしない、N1）。捕捉しないと、キュー済みキーが`nextRehydrateKey`に残留したまま関数を抜け、後の無関係な`rehydrateForRepo()`呼び出しの末尾でそのキーが「幽霊適用」される（settingsは最終リポなのにstore/DBは中間リポのまま）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |   A    |
| 12d | `rehydrateForRepo()`実行中に`runPendingRepoSyncIfIdle()`/`handleCloseSettings()`のアイドル判定（`shouldQueueRepoSync`）が走る（設定を閉じた直後・同期完了直後） | rehydrate完了を待ってから`isPulling`/`isPushing`/`isArchiveLoading`を読んでビジー判定する。待機中に他の同期がロックを取った場合はキューに積み、直接Pullは呼ばない                                                                                                                                                                                                                                                                      | `runPendingRepoSyncIfIdle()`（`git-pull.ts`）と`handleCloseSettings()`は`shouldQueueRepoSync`判定より前に`waitForRehydrate()`をawaitする（#297 should2）。待たずに判定すると、判定時点ではアイドルでも、待機中に取られたロックを見落として`pullFromGitHub()`を直接呼んでしまい、その内部の`waitForRehydrate()`待機後の`canSync`判定で黙って`return`する（`pendingRepoSync`も立てていないためキューにも積まれずPullが消える）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |   A    |
...
flowchart TB
    ...
    subgraph "関数レベルのガード"
        ...
        urlRestoreWait["restoreStateFromUrl()<br/>waitForSyncIdle()<br/>（#314 S1: signal通知で待機）"]
    end
    ...
    isPulling -->|待機| urlRestoreWait
    isPushing -->|待機| urlRestoreWait
    isPushingBackground -->|待機| urlRestoreWait
    isArchiveLoading -->|待機| urlRestoreWait
```
...
| `isArchiveLoading`       | `performArchiveLoad()` 冒頭（IndexedDB 読み出し前・同期。呼び出し元 `handleWorldChange()` / `restoreStateFromUrl()`）。`moveNoteToWorld()` / `moveLeafToWorld()` はアーカイブ事前Pull時に自前で取得 | `performArchiveLoad()` のfinally句。`moveNoteToWorld()` / `moveLeafToWorld()` は各関数自身のfinally句 |
...
2. `executeStaleCheck()` でリモート差分を確認（`check_failed` + `abortIfStaleCheckFailed` オプション時は中止。#235）
3. stale ならまず push-in-flight 救済（`tryRescueStalePush`）を試し、不成立なら 3 択ダイアログ（push / pull / cancel）（#235）
4. **送信用 snapshot を JSON deep clone（`JSON.parse(JSON.stringify(...))`）で固定** — `notes` / `leaves` / `metadata` / archive 系すべて（`structuredClone` を使わない理由は `git.ts` のコメント参照: Svelte 5 の `$state` proxy が DataCloneError を投げる経路がある）
5. `setPushInFlightAt(Date.now())`（設定した stamp を控えておく。#235）
...
- **#224: 自動消滅しない sticky トースト（`toast.pushInProgress`）を表示**し、送信完了前にアプリを切り替え／終了しないよう促す（`showStickyPushToast()`）。編集は続行できるが「もう保存完了」と誤解してバックグラウンド化・終了されると Push が宙に浮くため。Agasteer 内のペイン／ノート切替は止めないので、文言は「アプリの切り替えや終了」に限定。完了トースト（成功/失敗/タイムアウト）が後勝ちで差し替える。想定外の throw 経路だけは完了トーストが出ないので `clearPushToast()` で明示的に消す
- **#238: sticky トーストの2行目に FF 風カウントダウン（残りステージ数 5→1）を表示**する。`executePush` → `pushAllWithTreeAPI` の `onProgress` コールバックが各ステージ開始時に残数を通知する（ステージ対応表は `sync/push-stages.ts`。5=repo/ref 取得・4=base commit/tree 取得・3=新 tree 作成=本文アップロード・2=commit 作成・1=ref 更新。空リポジトリ初期化経路は「5」に含める）。表示は `setPushToastCountdown()` の**単調減少ガード**で絶対に増えない。orphan Push の遅延 `onProgress` は世代カウンタ（`pushProgressGeneration`）で弾き、タイムアウト経路では世代を進めて以降のコールバックを無効化する。表示層（`ui/ui.svelte.ts`）には**ペーシング**があり、各数字は最低 400ms（`PUSH_COUNTDOWN_MIN_HOLD_MS`）保持してキューで順に表示する。遅延対象は Push 完了専用入口 `showPushCompletionToast()`（通常完了・タイムアウト・orphan 遅延成功が使用）の success だけで、最後の数字の保持完了まで遅延する（最悪ケース: キュー4件+保持中で +2秒弱）。error は遅延せず即時表示・キュー破棄。汎用 `showPushToast()`（crud / move / share 等）は常に即時表示で、割り込んだ場合はキューだけ破棄し、遅延中の完了トーストは汎用トーストの2秒消滅時に必ず表示される（新しい Push の開始・clearPushToast・完了 error で上書きされた場合を除く）。git.ts / github.ts 側の通知タイミングは変えず、表示層だけの関心事とする

送信処理:

1. `Promise.race([executePush(snapshot), 30s timeout])`（#204 の救済機構と同じ）。タイムアウト時は `executePush` の Promise に `observeOrphanPush()` を接続し、遅延結果を観測する（#235）
...
live state を直接渡すと、Push 中にユーザーが編集した内容が executePush 内部で読まれて送信内容に混入する（参照同一性のため）。JSON deep clone で構造的変更（追加・削除・並べ替え・rename）からも保護する。
