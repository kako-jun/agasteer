...
| `lastKnownCommitSha`       | `writable<string\|null>`   | LocalStorageから復元（なければ`null`） | `result.commitSha`をセット                    | `result.commitSha`をセット                    | `null`（LocalStorageもクリア） | 旧リポのSHAと新リポのHEADが比較され、必ず「stale」と誤判定                    |
