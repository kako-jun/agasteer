...
| `metadata`                 | `$state<Metadata>`         | `{version:1,notes:{},leaves:{},pushCount:0}`（モジュール初期値。`initApp()`が`setCurrentRepo()`完了直後・stale check/Pullより前に`await getPersistedMetadata()`の結果で上書きする。以前はこの上書きを`applyPersistedStartupCache()`内でしか行っておらず、full pull経路でPullがonStructure到達前に失敗するとモジュール初期値のままinitStoreEffects()の初回実行に渡り、空metadataがIndexedDBへ書き戻る事故があった=#295 M1） | `metadataFromGitHub`で全体を上書き            | noChanges以外の成功時、`fetchRemotePushCount()`成功なら`pushCount`のみ更新（他フィールドは不変。#293） | `await getPersistedMetadata()`でIndexedDBの新リポキーから非同期再取得、なければ`{version:1,notes:{},leaves:{},pushCount:0}` | 旧リポのmetadata（pushCount含む）が新リポに残り、統計表示や衝突ダイアログのローカルpushCount比較が旧リポの値のまま誤表示される |
