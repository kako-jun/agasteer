...
| `metadata`                 | `$state<Metadata>`         | `{version:1,notes:{},leaves:{},pushCount:0}`（モジュール初期値。起動時に`await getPersistedMetadata()`の結果で非同期に上書き、#295） | `metadataFromGitHub`で全体を上書き            | noChanges以外の成功時、`fetchRemotePushCount()`成功なら`pushCount`のみ更新（他フィールドは不変。#293） | `await getPersistedMetadata()`でIndexedDBの新リポキーから非同期再取得、なければ`{version:1,notes:{},leaves:{},pushCount:0}` | 旧リポのmetadata（pushCount含む）が新リポに残り、統計表示や衝突ダイアログのローカルpushCount比較が旧リポの値のまま誤表示される |
