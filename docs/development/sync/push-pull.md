...
4. 保留中の自動保存をフラッシュ
5. Staleチェック（共通関数で時刻も更新）。`check_failed` のとき、`abortIfStaleCheckFailed` オプション指定時（auto-push）は中止（#235）
6. Stale編集の場合はまず push-in-flight 救済（`tryRescueStalePush()`）を試し、成立すればダイアログなしで続行。不成立なら確認（ロックを保持したまま await）（#235）
7. Push飛行中フラグを設定（`pushInFlightAt`。設定した stamp を控えておく）
8. **Push実行を `Promise.race` で 30 秒タイムアウト付きで待つ**（`PUSH_TIMEOUT_MS`、#204）
9. Push飛行中フラグをクリア（**タイムアウト時は意図的に残し**、`observeOrphanPush()` で遅延結果を観測する。#235）
10. 結果処理（成功時はダーティクリア、リモートからpushCount取得）
11. ロック解放（finallyで必ず実行）

**ステップ 8（30 秒タイムアウト）の意義（#204）:**
...
`pushToGitHub(options?: PushToGitHubOptions)` は `abortIfStaleCheckFailed` オプションを受け取る。`true` のとき、ステップ 5 の stale check が `check_failed`（ネットワーク断・認証エラー等で判定不能）なら Push を中止する。手動 Push（既定 `false`）は「ユーザーが押した」意思を尊重してそのまま続行するが、auto-push は無人発火のため中止し、オフライン中に 42 秒ごとのエラートースト連発になるのを防ぐ（auto-push の自前 stale check を preflight に一本化した後も、旧来の「check_failed は静かにスキップ」挙動を維持するためのオプション）。
