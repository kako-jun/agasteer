...
Pull失敗時のバックアップ復元は**初回Pull（`isInitialStartup = true`）では行わない**。

- **初回Pull失敗**: リストアしない。UIはガラス状を維持。オフラインリーフのみ操作可能。`online`イベントでオンライン復帰を検知し、自動でPullをリトライする
- **非初回Pull失敗**: 直前の同期済みデータからバックアップ復元する（作業中の状態を保護）

初回Pull時に古いデータをリストアすると、同期未完了のまま編集可能になり、オンライン復帰後のPullで上書きされるリスクがある。
