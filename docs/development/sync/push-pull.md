...
| `metadata`                 | `$state<Metadata>`         | `getPersistedMetadata()`、なければ`{version:1,notes:{},leaves:{},pushCount:0}` | `metadataFromGitHub`で全体を上書き            | (#293) noChanges以外の成功時、`fetchRemotePushCount()`成功なら`pushCount`のみ更新（他フィールドは不変） | 新リポのスロットから再読込、なければ`{version:1,notes:{},leaves:{},pushCount:0}` | 旧リポのmetadata（pushCount含む）が新リポに残り、統計表示や衝突ダイアログのローカルpushCount比較が旧リポの値のまま誤表示される |
