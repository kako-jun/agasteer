...
8. #297 should5: `RS中`（rehydrateForRepo実行中）に ArchiveLoad/WorldSwitch を開始しても拒否（ブロック）はしない。ワールド表示自体は即座に切り替わるが、`handleWorldChange()`/`moveNoteToWorld()`/`moveLeafToWorld()`のアーカイブロード（IndexedDB読み書き）は開始前に`waitForRehydrate()`でrehydrateの完了を待ってから実行される（旧/新DBの取り違え防止）。
   ...
   - #297 S-b: アーカイブロードのロック（`appState.isArchiveLoading = true`）は、この再判定を通過した直後・IndexedDB読出し（`loadArchiveCacheFromDB()`）を`await`する前に同期的に取得する。読出し完了後に取得すると、読出し中はロックが無い窓ができ、その間に割り込んだ Pull がアーカイブロードと並走できてしまう。
...
| `isArchiveLoading`                             | 各関数の冒頭で個別チェック（`handleWorldChange()`は再判定通過直後に同期的に取得。#297 S-b）                                                                                                                                                                                                                       | Pull, Push, WS, 移動           |
...
export function resetForRepoSwitch(): void {
  ...
  // lastPushed* は dirty-tracking.ts のモジュール private 変数のため、
  // resetPushedSnapshots() 経由でクリアする
