...
8. #297 should5: `RS中`（rehydrateForRepo実行中）に ArchiveLoad/WorldSwitch を開始しても拒否（ブロック）はしない。ワールド表示自体は即座に切り替わるが、`handleWorldChange()`/`moveNoteToWorld()`/`moveLeafToWorld()`のアーカイブロード（IndexedDB読み書き）は開始前に`waitForRehydrate()`でrehydrateの完了を待ってから実行される（旧/新DBの取り違え防止）。
   ...
   - #297 S-a: 再判定が`isPulling || isPushing || isPushingBackground`を理由に打ち切った場合だけは、Pull/Push はアーカイブをロードしないため、そのまま終わるとワールド表示は切り替わっているのにアーカイブが未ロード（`isArchiveLoaded=false`、画面が空）のまま残ってしまう。この場合は`appState.pendingArchiveLoad = true`を立てて打ち切り、同期完了後の`runPendingRepoSyncIfIdle()`（下記の一本化された実装）の末尾で`resumeArchiveLoadIfPending()`を呼び、対象ペインがまだ archive 表示かつ未ロードならアーカイブロードを自動的に再開する。ロード本体（IndexedDB読出し+`pullArchive`）は`handleWorldChange()`と`resumeArchiveLoadIfPending()`で共有する1関数に抽出済み（二重実装しない）。
...
| `pendingArchiveLoad`（#297 S-a）        | `resumeArchiveLoadIfPending()`（`pane-navigation.svelte.ts`、`runPendingRepoSyncIfIdle()`末尾から`appActions`経由で呼ばれる）。対象ペインが archive 表示でなくなった／既にロード済みならフラグだけ下ろす、まだ busy ならフラグを維持して次の完了フックに委ねる                                                    | アーカイブロードの自動再開     |
...
| 12b | `rehydrateForRepo()`実行中にアーカイブロード開始（`handleWorldChange`のワールド切替、`moveNoteToWorld`/`moveLeafToWorld`のWorld間移動）                         | rehydrate完了を待ってからアーカイブのIndexedDB読み書きが始まる。待機中に他の同期（Pull/Push/AL）がロックを取った、または別ペインのアーカイブロードが先に完了した場合はアーカイブロードを開始しない。**ただし Pull/Push が理由で打ち切った場合は、同期完了後に自動でアーカイブロードを再開する**（#297 S-a。対象ペインがまだ archive 表示かつ未ロードのときだけ。ペインが home に戻っていれば再開しない） | `moveNoteToWorld()`/`moveLeafToWorld()`は`waitForRehydrate()`を関数冒頭・Pull/Push/AL判定より前に置き、待機後の状態で判定を1回だけ行う。`handleWorldChange()`はワールド表示の即時切替（注8）を保つため、待機前に1回（表示切替の可否）、`waitForRehydrate()`をawait後にもう1回（ペインがまだ archive 表示か・設定の有効性・`isPulling`/`isPushing`/`isPushingBackground`/`isArchiveLoading`/`isArchiveLoaded`の再判定、#297 N-a）の計2回判定する（#297 must1）。判定→待機→再判定なしで進むと、`setCurrentRepo()`によるDB切替の途中でアーカイブの読み込み/保存が走り旧/新DBを取り違える、または待機中に取られた Pull とアーカイブロードが並走する（排他表崩壊）、連続 Archive 操作で AL が二重起動する。Pull/Push を理由に打ち切った場合の自動再開は`appState.pendingArchiveLoad`フラグ＋`resumeArchiveLoadIfPending()`（#297 S-a）。アーカイブロード完了時の保留同期消化（`runPendingRepoSyncIfIdle()`）は、以前は`handleWorldChange()`/`moveNoteToWorld()`/`moveLeafToWorld()`がそれぞれ`waitForRehydrate()`も`pendingRehydrateRepo`のrehydrateもしない簡略版を複製していたが、`git-pull.ts`の実装（正本。`pendingRehydrateRepo`があれば予約pull前にrehydrateする）に一本化した（#297 S-c。`git-push.ts`と同じ import 形） |   A    |
