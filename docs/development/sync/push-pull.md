...
| rehydrate実行中（`waitForRehydrate()`） | `pullFromGitHub()`/`pushToGitHub()`冒頭（`canSync`判定より前）、`handleWorldChange()`/`moveNoteToWorld()`/`moveLeafToWorld()`のアーカイブロード直前（#297） | Pull, Push, WS, 移動   |
...
| 12b | `rehydrateForRepo()`実行中にアーカイブロード開始（`handleWorldChange`のワールド切替、`moveNoteToWorld`/`moveLeafToWorld`のWorld間移動） | rehydrate完了を待ってからアーカイブのIndexedDB読み書きが始まる                                                                 | アーカイブロード直前で`waitForRehydrate()`をawait（#297 should5）。待たずに進むと、`setCurrentRepo()`によるDB切替の途中でアーカイブの読み込み/保存が走り、旧DBに書く・新DBから読むなど取り違える                                                                                                               |   A    |
| 12c | `applyRehydrateForRepo()`内の未保護await（`getPersistedMetadata()`等）が例外を投げた直後に別リポへの切替要求が来ている                  | 例外が起きてもキュー済みの最後の要求（最終的に必要なリポ）は破棄されず必ず適用される。キュー消化後に例外はまとめてrejectされる | `rehydrateForRepo()`が各周回の例外を捕捉して継続し、キューが空になるまで処理を続ける（#297 must1）。捕捉しないと、キュー済みキーが`nextRehydrateKey`に残留したまま関数を抜け、後の無関係な`rehydrateForRepo()`呼び出しの末尾でそのキーが「幽霊適用」される（settingsは最終リポなのにstore/DBは中間リポのまま） |   A    |
