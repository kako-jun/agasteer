...
| 範囲           | 関数                   | 説明                   |
| -------------- | ---------------------- | ---------------------- |
| E-1001〜E-1017 | `pushAllWithTreeAPI`   | Push処理のエラー       |
| E-2001〜E-2009 | `pullFromGitHub`       | Pull処理のエラー       |
| E-3001〜E-3009 | `testGitHubConnection` | 接続テストのエラー     |
| E-4001〜E-4009 | `pullArchive`          | アーカイブPullのエラー |

コードは `sync-handlers.ts` の `translateGitHubMessage` で翻訳済みメッセージに付与される。エラーコードの定義は各関数のreturn文に直接記述されている（`errorCode` フィールド）。
