...
「完全に操作不可」は視覚的なガラス効果だけでなく、背面の `<main class="main-pane">` に `inert` 属性を付与することで DOM レベルでも遮断される（エディタのキー入力、Priority リーフのリンクジャンプ等）。オフラインリーフを開いている間はガラス効果も `inert` も適用されないため、Push/Pull 中でも編集可能。
...
- 350件でレート制限: 1回目200件取得→失敗、2回目150件のみ取得→成功
...
- エラーメッセージをユーザーに表示（エラーコード付き）
- ネットワークエラーはtry-catchで捕捉

### エラーコード

各エラー返却箇所に一意のエラーコード（`E-xxxx`）が割り当てられている。ユーザーからの報告時にコードだけで発生箇所を特定できる。

| 範囲           | 関数                   | 説明                   |
| -------------- | ---------------------- | ---------------------- |
| E-1001〜E-1018 | `pushAllWithTreeAPI`   | Push処理のエラー       |
| E-2001〜E-2010 | `pullFromGitHub`       | Pull処理のエラー       |
| E-3001〜E-3009 | `testGitHubConnection` | 接続テストのエラー     |
| E-4001〜E-4009 | `pullArchive`          | アーカイブPullのエラー |
| E-5001〜E-5002 | `executePush`          | Push前バリデーション   |

エラーコードは各関数の return 文に `errorCode` フィールドとして定義されている。`sync-handlers.ts` の `translateGitHubMessage` が翻訳済みメッセージの末尾にコードを付与して表示する。

HTTPレスポンスが取得できたエラーでは、ステータスコードも併記される（例: `E-1002/403`）。
ネットワークエラーやバリデーションエラーなど、HTTPレスポンスがない場合はエラーコードのみ表示される。
