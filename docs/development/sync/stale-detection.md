...
2. stale編集かどうかチェック（`check_failed` のとき、auto-push 経由なら `abortIfStaleCheckFailed` オプションにより中止。#235。[`push-pull.md`](./push-pull.md) 参照）
3. staleの場合はまず push-in-flight 救済（`tryRescueStalePush()`、後述）を試す。成立すれば stale は誤検出なので、ダイアログを出さずにPushを続行（#235）
4. 救済不成立なら`isStale`を`true`にしてPullボタンに赤丸を表示
5. 3択ダイアログを表示（Pull first / Push上書き / Cancel）
6. Cancelした場合も赤丸を維持し、ユーザーがPullすべき状態を見失わないようにする
7. staleでなければそのままPush
...
- Push時のstaleチェック（手動 / 自動Push共通。#235で`pushToGitHub()`のpreflightに一本化）
- 定期チェック実行時
...
### 長時間バックグラウンド復帰時の動作（実サスペンド検出・#191）

`initApp` はハートビート interval（`setInterval`、約 30 秒間隔 = `HEARTBEAT_INTERVAL_MS`）で「プロセスが生きていた最終時刻」`lastHeartbeatTime` を刻み続ける。マシンが起きていればタブが hidden でも背景タイマーは走る（Chrome のスロットルでも最長〜60 秒で止まらない）ため、タブ切替・最小化では gap は〜30 秒に保たれる。`visibilitychange` で `visible` に復帰したとき `gap = now - lastHeartbeatTime` を計算し、`isLongSuspendGap(gap, 5 分)`（= `gap > 閾値`）が `true` のときだけ「長時間バックグラウンドにいました」モーダルを表示する。gap が閾値を超えるのは、interval が刻めないほどプロセスが凍結していたとき（OS スリープ / 長時間 frozen tab）だけで、単にタブを隠しただけでは発火しない。モーダルを閉じると stale チェックが走り、結果は周期 stale チェックと**完全に同じ共通関数 `applyStaleResult()`** で処理される（push-in-flight フラグの取り扱い含めて分岐は共通）。

以前は hidden だった実時間そのものを代理指標にしており、タブを 5 分以上隠しただけでも誤発火していた（#191 で gap ベースに修正）。ハートビート interval が閾値より十分短い間隔で gap を詰め続けるので、マシンが起きている限りどれだけ長くタブを隠してもモーダルは出ない。
...
1. Push APIリクエスト送信直前に`pushInFlightAt`タイムスタンプをLocalStorageに設定（#235: 設定した値=stampを`pushToGitHub`内で控えておく）
2. レスポンス受信後（成功・失敗問わず）にクリア。タイムアウト時は意図的に**残し**、後述の orphan Push 観測に引き継ぐ。orphan 側の遅延クリアは「自分が設定した stamp と一致する場合のみ」の **guarded clear**（後続 Push が新しい値を設定していたら、それはその Push の救済マーカーなので触らない。#235）
3. staleチェックでstale検出時にフラグが残っていれば、スリープ等によるレスポンス消失と判断

**救済ヘルパー `tryRescueStalePush()`（#235）:**

stale検出時の救済判定は、`src/lib/stores/stale-checker.svelte.ts` の共通ヘルパー
`tryRescueStalePush(result, logContext): boolean` に切り出されている。呼び出し経路は4つ:

1. 定期staleチェック（`applyStaleResult()` 経由）
2. visibility復帰時のstaleチェック（同上）
3. online復帰時のstaleチェック（同上）
4. **Push preflight（`pushToGitHub()` 自身）** — 救済成立時は stale は誤検出（リモートHEADは自分が送ったコミット）なので、衝突ダイアログを出さずにPushを続行する

preflight から `applyStaleResult()` を丸ごと呼ばないのは、stale + clean のとき
`shouldAutoPull` が立ち、Push中にPullが予約されてしまう副作用があるため。救済判定
だけをヘルパーに切り出しているのはそのため。
...
1. **Phase A: `pushToGitHub` 内で `Promise.race` による 30 秒タイムアウト**（`PUSH_TIMEOUT_MS`）
   ...
   - `pushInFlightAt` は意図的に**残し**、`observeOrphanPush()` で遅延結果を観測する（#235、次節）。ページリロード等で観測が消えた場合も、次回 stale check の `tryRescueStalePush` が保険として救済する
...
### タイムアウト後の orphan Push 観測（#235）

Phase A のタイムアウト後も `executePush` の fetch チェーンは生きており、遅れて完走するとリモート ref を force 更新する（orphan Push）。従来はこの結果を誰も観測せず「次回 stale check での救済」だけに頼っていたため、

- **遅延成功**: リモート HEAD が orphan のコミットになったのに `lastKnownCommitSha` は更新されず、`pushInFlightAt` も後続 Push の応答時に消されるため、以降の stale check が毎回 stale になり衝突ダイアログが出続けた（#235 の主症状）
- **遅延失敗**: 誰も catch せず unhandledrejection

という2つの穴があった。現在は race に負けた `executePush` の Promise に `observeOrphanPush(pushPromise, stamp)`（`src/lib/actions/git.ts`）で continuation を接続し、settle した瞬間に結果を反映する:

- **遅延成功（スロット一致）**: `pushInFlightAt` スロットが自分の stamp のまま（= 後続 Push は挟まっていない通常ケース）なら、`lastKnownCommitSha` を戻り値の commitSha に追従・`isStale = false`・成功トースト（`toast.pushLateSuccess`、ja/en）でタイムアウト通知を上書き。この場合に追従してよいのは、settle した瞬間のリモート HEAD がこのコミットで確定しているから（force push で上書き済み。ローカルは全リーフを保持しており Push は毎回全ツリーを送るため、次の Push でリモートの内容も復元される）。ただし別 Push が進行中（`isPushing || isPushingBackground`）ならトーストは出さない（進行中 Push の sticky トースト `toast.pushInProgress` を消さないため。SHA 反映は行う）
- **遅延成功（スロット不一致）**: 後続 Push が挟まった兆候。settle 順 ≠ ref 更新順があり得る（orphan の ref 更新 → 後続 Push B 成功 → orphan のレスポンス到着、の順だと盲目追従で `lastKnownCommitSha` が shaB → shaA に逆流し偽 stale が出る）ため盲目追従せず、`executeStaleCheck` を 1 回走らせて実リモート HEAD を確認する。stale かつ remoteCommitSha が orphan の commitSha と一致する（= リモート HEAD が自分の送ったコミットと確認できた）場合のみ `lastKnownCommitSha` を揃える。一致しない stale（第三者のコミットがリモート HEAD）は、揃えると真正な divergence を隠して次の Push が無警告上書きになるため何もせず、通常の stale フロー（次回チェック → ダイアログ）に委ねる。up_to_date なら何もしない。check_failed / 例外なら何もせず次回の定期チェックに委ねる
- **遅延失敗 / reject**: warn ログのみ（reject 経路は unhandledrejection の解消も兼ねる）
- いずれの場合も `pushInFlightAt` は「自分が設定した stamp と一致する場合のみ」クリアする（**guarded clear**。後続 Push が設定した救済マーカーを壊さないため）
- **リポ一致ガード**: `pushInFlightAt` / `lastKnownCommitSha` / `isStale` はいずれも「現在リポ」のスロットが対象のため、settle 時にリポが切り替わっていたら warn ログのみで全書き込み（SHA・isStale・トースト・フラグ）をスキップする（新リポの同期状態に旧リポの SHA を混入させない）。旧リポ側の `pushInFlightAt` は残るので、旧リポに戻ったときの `tryRescueStalePush`（下記の第2段）が救済する

救済は**2段構え**: 第一義はこの orphan 継続観測で、ページリロード等で continuation ごと消えた場合の保険として、従来どおり次回 stale check の `tryRescueStalePush` でも救済される。
...
- **orphan Push 観測**: タイムアウト後の遅延結果を settle 時に反映し、誤stale の温床を塞ぐ（#235）
...
| `pushInFlightAt`      | 「Push 開始時刻」。preflight phase 終端で `Date.now()` を立てる。background phase の正常終了で消費し、タイムアウト時は残して orphan 観測（#235）の settle または次回 stale check の救済で消費される |
...
- 30 秒タイムアウトは background phase の `Promise.race` 内で発火し、`isPushingBackground=false` で UI ロックを解除する（`pushInFlightAt` は意図的に残し、`observeOrphanPush` が遅延結果を観測する。#235）
