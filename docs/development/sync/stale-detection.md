...
PWA が 5 分以上バックグラウンドにあった状態から復帰すると、`visibilitychange` ハンドラが「長時間バックグラウンドにいました」モーダルを表示する。モーダルを閉じると stale チェックが走り、その後の挙動は **上記の周期 stale チェックと同じ**:

| 状態                | 処理                                   |
| ------------------- | -------------------------------------- |
| stale & クリーン    | `shouldAutoPull = true` → 自動Pull実行 |
| stale & ダーティ    | `isStale = true`（赤バッジ）           |
| up_to_date          | `isStale = false`                      |
| pushInFlightAt あり | 後述の Push 飛行中フラグ処理           |
