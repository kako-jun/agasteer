...
- Archiveを選択すると**即座にアーカイブ画面に切り替わる**
- バックグラウンドでGitHubからデータがPullされる
- 右下の統計表示（リーフ数・文字数）がリアルタイムで増加し、進捗を確認できる
- 2回目以降はキャッシュ済みで即座に表示

### 切り替え制限

以下の状況では、ワールドの切り替えがブロックされます：

- **Pull中**（ホームのデータを取得中）
- **Push中**（GitHubに保存中）
- **アーカイブロード中**（アーカイブのデータを取得中）

> **💡 Tip**: データの不整合を防ぐため、同期処理中は操作が制限されます。処理が完了するまでお待ちください。
...
- リーフをアーカイブ → 操作後は元の親ノートに遷移（削除と同じ。連続アーカイブしやすい）
- ノートをアーカイブ → 操作後は元の親ノートに遷移
- アーカイブ先を見たいときは breadcrumb 先頭の `›` から world を切り替える
