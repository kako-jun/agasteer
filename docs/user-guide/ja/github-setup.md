...
> **📎 ファイル添付を使う場合**: 添付ファイルは `{リポジトリ名}-media` という別のプライベートリポジトリに保存されます。Fine-grained トークンにはリポジトリ作成の権限がないため、**`{リポジトリ名}-media` リポジトリも自分で作成し、③の「Repository access」で両方のリポジトリを選択**してください（Classic トークン（`repo` スコープ）の場合は初回添付時に自動作成されるため不要です）。
