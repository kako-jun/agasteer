...
- The media repository (`{your-repo-name}-media`) is auto-created on the first attachment, but that requires a token that can create new repositories. A minimal **fine-grained token** (selected repositories only, Contents permission only) cannot create it, so the attachment fails with `HTTP 403 Resource not accessible by personal access token`. For how to choose a token when using attachments (let a classic token auto-create it, or create the media repo yourself with a fine-grained token), see [Section 2 "Obtain a Personal Access Token" in the GitHub Integration Setup Guide](../github-setup.md#2-obtain-a-personal-access-token)
