# Customization

> Customize Agasteer's appearance to your liking.

---

## Appearance

- **[Themes](./customize/theme.md)** - Choose from 6 themes
- **[Lined Mode](./customize/lined-mode.md)** - Display lines like a paper notebook
- **[Custom Fonts](./customize/font.md)** - Upload your favorite fonts
- **[Custom Background Images](./customize/background.md)** - Set backgrounds for left/right panes separately

## Other

- **[Language Settings](./customize/language.md)** - Japanese / English
- **[Tool Name](./customize/toolname.md)** - Change the app name in the header

## Reference

- **[Settings Storage Location](./customize/save-location.md)** - Where each setting is saved

---

## Next Steps

Make use of convenient features.

→ [Useful Features](./features.md)
