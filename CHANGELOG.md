...
- **Vimモード**
  - CodeMirror Vim拡張を導入
  - 設定画面で切り替え可能
  - `:w`でPush、`:q`で親ノートへ遷移、`:wq`で保存して閉じる
  - `<Space>`でペイン切り替え（2ペイン対応）

- **罫線エディタモード**（#18）
  - 設定で有効化、ノート風罫線と行番号を表示
  - ライト/ダーク両テーマ対応

- **編集/プレビュー間のスクロール同期**
  - 同じリーフを左右で開いた際の双方向スクロール同期
  - 無限ループ防止機構
...
- **Offlineリーフ**
  - Pull中でもオフラインリーフは編集可能
  - GitHub同期と独立

### GitHub同期

- **Git Tree APIによる一括Push**
  - 削除・リネーム対応
  - APIリクエスト数削減
  - SHA最適化（変更されていないファイルは転送しない）

- **Pull高速化**（優先度ベースの段階的ローディング）
  - URLで指定されたリーフを第1優先で取得
  - 10並列でリーフを取得
...
- **アーカイブ機能**
  - Home/Archive切り替えUI
  - アーカイブ/リストアボタン、階層維持で移動

- **ノート/リーフの移動機能**
  - MoveModal、ドラッグ&ドロップ、移動先選択
...
- **ペイン操作メニュー**
  - 左右入れ替え、片方を閉じる、同じリーフを開く

- **シェア機能**
  - パンくずリストからURLコピー、Markdownコピー、QRコード表示

- **プレビュー画像ダウンロード/シェア**
  - html2canvas動的ロード、白背景+20px余白
  - Clipboard APIで画像コピー、Web Share APIでLINE等に直接送信

- **新規ノート/リーフ作成時の名前入力モーダル**
  - フッターボタンクリックでモーダル表示
  - Enter/Escキー対応

- **初回ガイド**（#19）
  - ノート/リーフ作成ボタン、保存ボタンへの吹き出し表示

- **ホーム統計拡張**（#15）
  - リーフ数・文字数・Push回数を右下に表示

- **タイトルリンク化**
  - Ctrl+クリックで別タブを開く、中クリック対応

- **GitHub設定ヘルプ**
  - リポジトリ名・トークン入力欄に?アイコン、モーダルで説明画像表示
...
- **PWA更新の即時適用**
  - onNeedRefreshで更新検知、初回Pullより先にリロード
...
- **ボタン共通化**
  - IconButtonコンポーネント、14個のアイコンコンポーネント
  - コード重複削減

- **ビルド日付の自動生成**
  - Vite define、ビルド時に自動的にバージョン番号を日付に設定

- **システム等幅Webフォント**
  - BIZ UDGothic自動ダウンロード、IndexedDBキャッシュ
  - エディタ+codeブロックのみ適用
